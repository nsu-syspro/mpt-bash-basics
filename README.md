...
в этом задании. Вы могли заметить, что помимо прохождения тестов, запускается
еще один "workflow" под названием `Pre-check`, в котором есть "job" `Check solutions`,
который запускает `ShellCheck` на всех скриптах в директории `solution`.
